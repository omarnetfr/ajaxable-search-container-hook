...
<aui:script use="aui-base,aui-io-plugin">
    ...
    var instance = '<%= namespace + id %>';
   
    var namespace = '<%= namespace %>';
    
    var curParam = '<%= curParam %>';
    
    var deltaParam = '<%= deltaParam %>';
  
    var paginatorContainer = A.one('#' + instance);
 
    if ( paginatorContainer ) {
  
        var placeholder = paginatorContainer.get('parentNode');
        var placeholderInstance = null;
  
        if ( instance.search(/PageIteratorTop|PageIteratorBottom/i) !== -1 ) {
  
            placeholderInstance = instance.replace(/PageIteratorTop|PageIteratorBottom/i, 'SearchContainer');
          
            placeholder = A.one('#' + placeholderInstance);
          
            if ( placeholder ) {
                placeholder = placeholder.get('parentNode');
            } else {
                placeholder = paginatorContainer.get('parentNode').get('parentNode');
                
                if (placeholder.hasAttribute('id')) {
                	placeholderInstance = placeholder.getAttribute('id');
                } else {
                	placeholderInstance = instance + 'placeholder';
                	placeholder.setAttribute('id', placeholderInstance);
                }
            }
        ...
        if ( placeholder ) {
                ...
                A.Plugin.IO, {
                    ...
                    after: {
                        success: function(event) {
                            ...
                            if ( placeholder ) {
                                data = placeholder.one('#' + placeholderInstance);
                            }
                           
                            placeholder.setContent(data.get('parentNode'));
            ...
            paginatorContainer.all('a').on(
                ...
                function(event) {
                    ...
                    placeholder.io.set('uri', uri);
                    placeholder.io.start();
                 
                }
            );

            var page = A.one('#' + instance + '_page');
  
            if (page) {
                page.removeAttribute('onchange');
                page.detachAll();
                ...
                page.on(
                    ...
                    function(event) {
                        ...
                        var uri = paginatorContainer.one('.first').getAttribute('href');
                                 
                        if (uri === '') {
                            uri = paginatorContainer.one('.last').getAttribute('href');
                        }
  
                        uri = uri.replace(/p_p_state=normal/i, 'p_p_state=exclusive');
                       
                        var regex = new RegExp(curParam + '=(.*)', 'g');
                        uri = uri.replace(regex, curParam + '=' + cur);
            ...
            var itemsPerPage = A.one('#' + instance + '_itemsPerPage');
  
            if (itemsPerPage) {
                itemsPerPage.removeAttribute('onchange');
                itemsPerPage.detachAll();
                ...
                itemsPerPage.on(
                    ...
                    function(event) {
						...
						regex = new RegExp(namespace + deltaParam, 'g');
						
                        if (uri.search(namespace + deltaParam) === -1) {
                            uri = uri + '&' + namespace + deltaParam + '=' + delta;
                        } else {
                            regex = new RegExp(namespace + deltaParam + '=(.*)', 'g');
                            uri = uri.replace(regex, namespace + deltaParam + '=' + delta);
                        }
